...
import { MDXImage, Callout, Quote, Metric, VideoEmbed, FigmaEmbed, ProductTrioDiagram, Ref, Footnotes, FootnoteItem, BeforeAfter, AnimatedMetricsDashboard, MediaFrame, RxProAiDemo, AfyaDiscovery, IntelieBottleneck } from "@/components/mdx";
...
const mdxComponents = {
    ...
    h3: (props: any) => {
        const textContent = React.Children.toArray(props.children).join('');
        return <h3 id={slugify(textContent)} className="text-xl md:text-2xl font-medium mt-8 mb-4 text-foreground scroll-mt-32" {...props} />;
    },
                ...
                {blocksToRender && blocksToRender.length > 0 ? (
                    blocksToRender.map((block: any, index: number) => {
                        switch (block.type) {
                            ...
                            case 'intelie-bottleneck':
                                return (
                                    <div key={index} className="my-12 w-full">
                                        <MediaFrame className="flex flex-col w-full bg-background border-2 border-foreground relative">
                                            <IntelieBottleneck locale={requestedLocale} />
                                        </MediaFrame>
                                        {block.alt && <figcaption className="text-center !text-[11px] text-foreground type-label !mt-8 max-w-xl mx-auto">{block.alt}</figcaption>}
                                    </div>
                                );
