...
export default function ContactPage({ params: { locale } }: { params: { locale: string } }) {
    ...
    return (
        <div className="container mx-auto px-4 py-20 max-w-xl text-center">
            <h1 className="text-4xl font-semibold mb-2">{content.pageTitleHeader.title}</h1>
            {content.pageTitleHeader.subtitle && (
                <h2 className="text-xl text-foreground font-light mb-12">{content.pageTitleHeader.subtitle}</h2>
            )}
            ...
            {content.contactOptions.items.length > 0 && (
                <div className="space-y-6">
                    {content.contactOptions.items.map((item, i) => (
                        ...
                        >
                            <h3 className="text-xl font-semibold mb-2 text-foreground">{item.platformName}</h3>
                            <p className="text-foreground font-light mb-4">{item.description}</p>
