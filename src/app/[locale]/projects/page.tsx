...
export default function ProjectsPage({ params: { locale } }: { params: { locale: string } }) {
    ...
    return (
        <div className="container mx-auto px-4 py-20">
            <h1 className="text-4xl font-semibold mb-12 text-center">{locale === 'pt' ? 'Cases' : 'Cases'}</h1>
                ...
                {projects.map((project, i) => {
                    ...
                    return (
                        <Link href={href} key={i} className="group block">
                            <div className="relative h-64 bg-halftone overflow-hidden mb-4 rounded-lg">
                            ...
                            <h3 className="text-xl font-semibold mb-2">{project.meta.title}</h3>
                            <p className="text-foreground font-light">{project.meta.summary}</p>
                        </Link>
                    );
                })}
            </div>
