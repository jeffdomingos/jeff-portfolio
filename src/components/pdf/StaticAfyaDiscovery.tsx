...
const MiroFrame = ({ title, children, className = "" }: { title?: string, children: React.ReactNode, className?: string }) => (
  <div className={`relative border-2 border-foreground/10 bg-white p-4 mt-4 ${className}`}>
...
const SmallPostIt = ({ children, colorClass, className = "", stacked = false }: { children: React.ReactNode, colorClass: string, className?: string, stacked?: boolean }) => (
  <div className="relative mx-auto group">
    {stacked && (
      <>
        <div className={`absolute inset-0 ${colorClass} w-24 h-24 shadow-[1px_1px_2px_rgba(0,0,0,0.1)] transform rotate-6 opacity-70`}></div>
        <div className={`absolute inset-0 ${colorClass} w-24 h-24 shadow-[1px_1px_2px_rgba(0,0,0,0.1)] transform -rotate-3 opacity-90`}></div>
      </>
    )}
    <div className={`relative ${colorClass} w-24 h-24 p-2 shadow-md text-[10px] text-foreground/90 font-medium leading-tight flex items-center justify-center text-center transition-transform group-hover:-translate-y-1 z-10 ${className}`}>
      <div>{children}</div>
    </div>
  </div>
);

export function StaticAfyaDiscovery({ locale = 'pt' }: { locale?: string }) {
  const isPt = locale === 'pt';

  return (
    <div 
      className="w-full bg-neutral-100 p-4 relative overflow-hidden" 
      ...
      <div className="flex flex-col gap-4">
        ...
        <MiroFrame>
          <div className="grid grid-cols-3 gap-6">
            ...
            <div>
              <h4 className="text-xs font-bold text-foreground uppercase mb-2 tracking-widest text-foreground/70 text-left">
            ...
            <div>
              <div className="relative inline-block w-full text-left">
                <h4 className="text-xs font-bold text-foreground uppercase mb-2 tracking-widest text-foreground/70 inline-block relative">
                  Pharma Partner (MVP)
                  {/* FigJam Style Checkmark Sticker */}
                  <div className="absolute -top-4 -right-8 w-7 h-7 rounded-full bg-emerald-500 text-white flex items-center justify-center shadow-md border-[3px] border-white z-10 rotate-12 hover:-rotate-3 transition-transform cursor-pointer" title="Selected MVP">
        ...
        <div className="w-full relative h-[130px]">
             ...
             <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-row items-center justify-center w-[740px] gap-2 transform scale-[0.60]">
